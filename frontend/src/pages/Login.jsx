...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (success) {
      ...
      setError('Invalid physician credentials.');
    }
  };
      ...
      <div className="relative w-full max-w-md bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-indigo-200">Physician Access Code</label>
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-indigo-500 to-sky-500 hover:from-indigo-400 hover:to-sky-400 text-white font-medium py-3 px-4 rounded-xl shadow-lg shadow-indigo-500/25 transition-all active:scale-[0.98]"
          >
            Authenticate
          </button>
        </form>
        ...
        <div className="mt-8 text-center">
          <p className="text-indigo-300/60 text-xs">
            Demo Access Code: <span className="text-indigo-200 font-mono bg-white/5 px-2 py-1 rounded">asthmaguard2026</span>
          </p>
        </div>
      </div>
    </div>
